...
puppet var puppet_hp = 150 setget puppet_hp_set
...
func _network_peer_connected(id) -> void:
	...
	rset_id(id, "puppet_skin_name", skin_name)
...
sync func heal_from_pack(amount):
	hp = min(hp + amount, 150)
	set_hp(hp)
...
func update_shoot_mode(shoot_mode):
	...
	var sprite_y = get_skin_region_for_player(skin_name)
	if shoot_mode:
		sprite.set_region_rect(Rect2(758, sprite_y, 256, 250))
	else:
		sprite.set_region_rect(Rect2(0, sprite_y, 256, 250))

	can_shoot = shoot_mode


func update_shoot_mode1(shoot_mode):
	if not shoot_mode:
